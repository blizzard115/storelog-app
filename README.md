...
StoreLogは、アパレル店舗向けの売り場ノートアプリです。
...
売り場の連絡事項・入荷情報・クレーム対応・接客メモを共有し、既読管理・未読可視化・店舗ごとのデータ分離により、店舗内の情報共有を効率化します。
...
* `demo.shibuya.staff@example.com` / `password` でログインすると、渋谷店の投稿のみ確認できます
* 重要投稿、投稿種別、既読・未読状態を確認できます
* `demo.shinjuku.staff@example.com` / `password` でログインすると、新宿店の投稿のみ確認できます
* `demo.join@example.com` / `password` でログインし、店舗設定画面で店舗コード `DEMO-SHIBUYA` を入力すると、既存店舗へ参加できます
...
投稿者が店舗移動により現在は別店舗に所属している場合は、投稿者名のみを表示し、ユーザー詳細へのリンクは表示しないようにしています。
これにより、店舗ごとのデータ分離を維持しながら、404になる投稿者リンクを表示しないようにしています。
...
* 前後に空白がある店舗コードでも、空白を除去して参加できる
* 空の店舗コードでは所属店舗が変更されず、エラーが表示される
* 現在所属している店舗コードを入力してもエラーにならず、所属店舗が維持される
* 店舗変更後も過去の投稿の所属店舗は変わらない
* 店舗移動後のユーザーページには元店舗の投稿が表示されない
* 現在所属している店舗の投稿のみ表示される

投稿者リンク表示のrequest specでは、以下の動作を確認しています。

* 投稿者が同じ店舗に所属している場合は、投稿者リンクが表示される
* 投稿者が別店舗へ移動済みの場合、投稿詳細で投稿者名は表示されるがリンクは表示されない
* 投稿者が別店舗へ移動済みの場合、投稿一覧で投稿者名は表示されるがリンクは表示されない
